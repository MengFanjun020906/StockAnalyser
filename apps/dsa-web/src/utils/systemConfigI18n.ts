...
const fieldTitleMap: Record<string, string> = {
  ...
  NEWS_EVENT_EXTRACTOR_MODE: '新闻事件抽取模式',
  NEWS_EVENT_EXTRACTOR_MODEL: '新闻事件抽取模型',
  NEWS_EVENT_EXTRACTOR_TIMEOUT_SECONDS: '新闻事件抽取超时（秒）',
  NEWS_EVENT_EXTRACTOR_MAX_TOKENS: '新闻事件抽取 Token 上限',
  NEWS_EVENT_EXTRACTOR_TEMPERATURE: '新闻事件抽取温度',
...
const fieldDescriptionMap: Record<string, string> = {
  ...
  ANYSEARCH_API_KEY: '统一用于股票新闻、市场复盘、Agent 检索和通用网页搜索的 AnySearch 密钥。',
  ...
  NEWS_EVENT_EXTRACTOR_MODE: '结构化新闻事件抽取模式：fallback 只用规则兜底；llm 优先用轻量 LLM JSON 抽取；auto 仅在模型运行时可用时启用 LLM。',
  NEWS_EVENT_EXTRACTOR_MODEL: '用于新闻事件事实抽取的 LiteLLM 模型。推荐 deepseek/deepseek-v4-flash。',
  NEWS_EVENT_EXTRACTOR_TIMEOUT_SECONDS: '单次新闻事件 LLM 抽取请求的超时时间。',
  NEWS_EVENT_EXTRACTOR_MAX_TOKENS: '单次新闻事件 LLM 抽取的最大输出 token 数。',
  NEWS_EVENT_EXTRACTOR_TEMPERATURE: '新闻事件抽取采样温度。建议保持 0，减少 JSON 结构漂移。',
