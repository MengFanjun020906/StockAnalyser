...
市场 Regime 识别先判断方向，再把波动作为仓位和确认条件处理。高波动上涨趋势不等同于纯风险环境：它会降低动量席的初始仓位、要求承接确认或回踩不破，但不能仅因 `high_vol` 把强势候选踢出深挖。只有 `risk_off`、`panic` 或方向不明的极端波动，才会显著压低突破/接力候选优先级。
...
“不追高”是按 setup 分层的执行约束。回踩/低吸计划仍严格受 MA5/关键均线乖离约束；突破、涨停打开或资金接力计划不能只因高乖离直接淘汰，但必须写成条件入场，要求竞价或盘中承接、分歧转一致、关键价位不破、明确止损和更小首仓。一字涨停或明显不可成交的计划仍视为不可执行。
