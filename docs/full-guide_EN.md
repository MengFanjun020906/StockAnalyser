...
| `ENABLE_CHIP_DISTRIBUTION` | Enable chip distribution analysis. `get_chip_distribution` prefers Tushare `cyq_chips`, then keeps structured diagnostics and falls back to manager-backed probing. Chip probes use a dedicated timeout worker pool instead of competing with fundamental calls. GitHub Actions users must set `ENABLE_CHIP_DISTRIBUTION=true` in Repository Variables to enable; disabled by default in workflows. | `true` | Optional |
...
> - If a configured StockAPI token lacks permission for the hot-sector endpoint, that endpoint is retried anonymously. Exhausted free quota still degrades normally. StockAPI exception URLs redact `token` and `api_key` query parameters before entering logs or traces.
> - Market-regime and forward-probability tools require a sufficient index history. A short cache no longer blocks Sequoia or Baostock fallback, and `000300` can use local `000300.SH` bars instead of remaining stuck on a one-row cache.
