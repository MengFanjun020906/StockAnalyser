...
> - Update the Sequoia candidate DB with `python scripts/update_sequoia_candidates.py --trading-days 260`; it pulls recent A-share daily bars from baostock and writes each symbol into the SQLite DB pointed to by `SEQUOIA_CANDIDATE_DB_PATH`. Interrupted runs are resumable by default: rerunning skips symbols that already reach the latest local date and continues with the remaining symbols. Use `--no-incremental --no-resume` only when you need a full refresh from the first symbol.
> - `get_capital_flow` uses Tushare `moneyflow` by default and maps the latest available `net_mf_amount` plus 5-day/10-day sums into `main_net_inflow`, `inflow_5d`, and `inflow_10d`; when Tushare is unavailable it falls back to StockAPI historical `codeFlow`. Without `STOCKAPI_TOKEN`, the fallback can only query delayed history windows under the free quota and exposes the data date via `latest_date`.
