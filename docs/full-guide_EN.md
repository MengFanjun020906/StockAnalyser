...
| `AGENT_SELECTION_DEEP_DIVE_LIMIT` | Maximum L1 `watchlist_scan` candidates that enter stock-level deep-dive analysis; the default deep-dives 4 names as a balance between coverage and latency. Higher values improve coverage but increase tool calls and latency. | `4` | Optional |
