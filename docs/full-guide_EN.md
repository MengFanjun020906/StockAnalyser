...
| `TUSHARE_HTTP_URL` | Tushare-compatible HTTP endpoint. Defaults to the project private gateway and can be overridden per deployment. | `http://118.89.66.41:8010/` | Optional |
...
| `ENABLE_CHIP_DISTRIBUTION` | Enable chip distribution analysis. `get_chip_distribution` prefers Tushare `cyq_chips`, then keeps structured diagnostics and falls back to manager-backed probing when unavailable. GitHub Actions users must set `ENABLE_CHIP_DISTRIBUTION=true` in Repository Variables to enable; disabled by default in workflows. | `true` | Optional |
...
| `AGENT_SEED_FACT_TOOL_TIMEOUT_SECONDS` | Per-tool budget for `SeedFactPacket` collection; failures are written to trace instead of being treated as successful facts. | `30.0` | Optional |
...
| `SEQUOIA_CANDIDATE_DB_PATH` | Sequoia-style quantitative candidate SQLite path. `watchlist_scan` candidate discovery reads `stock_daily(symbol,date,open,high,low,close,volume,turnover)` and runs pattern strategies. Agent history/K-line tools also read this local table when the main DB has no daily bars. | `Sequoia-X/data/sequoia_v2.db` | Optional |
...
| `AGENT_CAPITAL_FLOW_TIMEOUT_SECONDS` | Timeout budget for explicit Agent `get_capital_flow` calls; the Agent path probes Tushare `moneyflow_dc`, `moneyflow_ths`, and legacy `moneyflow` concurrently within this budget, selects the first usable source by priority, and falls back to StockAPI `codeFlow` only when all Tushare sources fail; non-selected Tushare sources are audited best-effort in the background without delaying the main response | `30.0` | Optional |
| `get_board_capital_flow` | Agent tool: combines Tushare `moneyflow_ind_dc`, `moneyflow_ind_ths`, and `moneyflow_cnt_ths` industry/concept/region board capital-flow sources, normalizes amounts to CNY, and keeps `flow_sources/source_definitions` to prevent DC/THS semantic mixing | - | Tool |
| `get_tushare_stk_factor` | Agent tool for Tushare stock technical factors (`stk_factor`), returning MACD, KDJ, RSI, BOLL, CCI and related indicators calculated from front-adjusted prices; included in planning technical analysis and the reversal, momentum, quality-repair, and theme-catalyst desk manifests | - | Tool |
| `get_tushare_moneyflow_ind_ths` | Agent tool for THS industry money flow (`moneyflow_ind_ths`), supporting `trade_date` or `start_date/end_date`; converts original 100m-yuan amount fields to CNY and is included in the theme-catalyst and momentum desk tool manifests | - | Tool |
| `get_tushare_moneyflow_mkt_dc` | Agent tool for Eastmoney broad-market capital flow (`moneyflow_mkt_dc`), supporting `trade_date` or `start_date/end_date`; returns SH/SZ index moves plus market main/extra-large/large-order flow and is included in the reversal, momentum, and theme-catalyst desk manifests | - | Tool |
| `STOCKAPI_TOKEN` | StockAPI historical capital-flow token. `get_capital_flow` uses `stockapi.com.cn/v1/base/codeFlow` only as fallback when Tushare `moneyflow` is unavailable; blank uses the limited free quota, which only supports delayed history windows and very few daily requests. | - | Optional |
| `STOCKAPI_URL` | StockAPI historical `codeFlow` endpoint URL. Defaults to `https://www.stockapi.com.cn/v1/base/codeFlow`; override it for private proxies or test deployments. | Official codeFlow URL | Optional |
| `AGENT_SELECTION_DEEP_DIVE_LIMIT` | Maximum L1 `watchlist_scan` candidates that enter stock-level deep-dive analysis; valid range is 1-5. The default deep-dives 4 names as a balance between coverage and latency. Higher values improve coverage but increase tool calls and latency. | `4` | Optional |
| `AGENT_TOOL_CALL_TIMEOUT_SECONDS` | Timeout for one Agent tool-call batch; slow tools degrade as tool failures instead of exhausting the full Trace run. Diagnostic-heavy tools such as `discover_watchlist_candidates`, `detect_market_regime`, `get_capital_flow`, `get_chip_distribution`, `get_tushare_financial_indicators`, and `get_volume_analysis` get higher outer wait floors so they can return structured diagnostics instead of being cut off by the 30s shell timeout. | `30.0` | Optional |
...
Final Agent dashboards run a fact guard before being returned. Exact capital-flow values must match a successful `get_capital_flow` source chain, exact chip values must come from `get_chip_distribution`, and exact MA values must come from `calculate_ma` or `analyze_trend`. Missing evidence is rendered as `N/A` and recorded in `data_quality_warnings`. A-share buy quantities are also checked against the 100-share board-lot rule; odd-lot buy suggestions are replaced with "按100股整数倍".
...
> - Update the Sequoia candidate DB with `python scripts/update_sequoia_candidates.py --trading-days 260`; it pulls recent A-share daily bars from baostock, writes each symbol into the SQLite DB pointed to by `SEQUOIA_CANDIDATE_DB_PATH`, and also stores the Shanghai Composite benchmark as `000001.SH` for Seed Pool quality Alpha evaluation. Interrupted runs are resumable by default: rerunning skips symbols that already reach the latest local date and continues with the remaining symbols. Use `--no-incremental --no-resume` only when you need a full refresh from the first symbol.
...
> - `get_tushare_stk_factor` returns MACD/KDJ/RSI/BOLL/CCI from Tushare `stk_factor`, calculated on front-adjusted prices. Its front-adjusted bars are historical daily snapshots and may differ from dynamic `pro_bar` qfq output or locally recalculated indicators. Cite the date and source when using it in reports.
> - `discover_watchlist_candidates` and `detect_market_regime` are multi-component heavy tools. The Agent Runner gives them a higher outer wait floor so they can return structured inner diagnostics instead of being cut off by the generic 30-second tool shell timeout.
> - `get_capital_flow` fails over through Tushare `moneyflow_dc` (Eastmoney semantics), `moneyflow_ths` (THS semantics), and legacy `moneyflow`, returning as soon as the first usable per-stock source succeeds. Raw amount fields are in 10k CNY and the tool exposes CNY. `selected_flow_source` tells which source populates the top-level fields, and `flow_sources` retains the selected successful source with its own semantics. Top-level `main_net_inflow/main_inflow_5d/main_inflow_10d` always uses the selected source's documented main/large-order definition; `net_inflow*` uses the selected source's own net-flow definition; `inflow_5d/inflow_10d` remain compatibility aliases for the selected main-flow definition. Do not mix DC, THS, and legacy moneyflow numbers as if they shared one statistical definition; check `selected_flow_source`, `main_inflow_definition`, and `net_inflow_definition` first. After the main response, a single-threaded background audit compares non-selected Tushare sources best-effort; date, direction, or magnitude conflicts are written only to `warnings/source_conflicts/capital_flow_audit`, never overriding top-level capital-flow fields or delaying the main path. When all Tushare sources are unavailable, it falls back to StockAPI historical `codeFlow`. Without `STOCKAPI_TOKEN`, the fallback can only query delayed history windows under the free quota and exposes the data date via `latest_date`.
> - `get_tushare_moneyflow_mkt_dc` is the Eastmoney broad-market capital-flow source for market liquidity context. It does not replace stock-level `get_capital_flow` or board-level `get_board_capital_flow`, and its amounts must not be added to stock or board flow numbers.
> - `get_board_capital_flow` is the unified board-flow entry point. It combines Eastmoney `moneyflow_ind_dc` (industry/concept/region), THS `moneyflow_ind_ths` (industry), and THS `moneyflow_cnt_ths` (concept) in one response. The tool normalizes units and field names only; it does not add different statistical definitions together. Compare board flows only after checking `selected_flow_source`, `source_definitions`, and `flow_sources`. Use `get_tushare_moneyflow_ind_ths` when the model needs the THS industry source alone; it is included in the theme-catalyst and momentum desk `tools_manifest` files.
...
> - `FUNDAMENTAL_STAGE_TIMEOUT_SECONDS=8.0` indicates the target budget for the newly added fundamental stage, not a strict hard SLA.
...
- API entrypoints (`python server.py` or `python main.py --serve-only`) keep uvicorn / FastAPI logs in `logs/api_server_YYYYMMDD.log` and its debug file.
- WebUI entrypoint (`python webui.py`) writes to `logs/web_server_YYYYMMDD.log` and its debug file.
...
- `search_openinvest_news` wraps `openInvest/services/news_sources` as an Agent tool. It uses `yfinance.Ticker.news` by default for ticker-linked news, while RSS and DDGS are optional sources. DDGS requires extra dependencies such as `ddgs`; missing dependencies or per-source failures are reported in `source_chain` instead of failing the whole tool.
...
- Each trace also writes local backend artifacts under `data/agent_traces/<timestamp>-<session_id>/`, including `request.json`, `context.json`, `planner.json`, `events.ndjson`, `tool_calls.json`, `evidence_ledger.json`, `debate.json`, `final.md`, `todo.md`, and `summary.json`. When `watchlist_scan` uses the staged stock-selection pipeline, the trace also writes `stock_selection.json`, `selection_context.json`, `final_report.json`, and per-stage JSON artifacts for candidate discovery, screening, deep dives, allocation, adversarial review, and Judge decisions. Stock selection also writes `candidate_evidence.json` and `candidate_evidence.md`, a balanced evidence packet with up to 2 strategy, 2 news, 2 capital, and 2 fundamental candidates for downstream planning and troubleshooting reuse. Candidate-level evidence collection runs in parallel; duplicate stocks across dimensions keep the earlier bucket assignment, skip the duplicate, and continue filling from later candidates. The page status bar shows the current `Artifact` path. The directory lives under `/data/` and is ignored by Git by default.
- The planning prompt includes a dedicated `Execute Protocol`: the executor must turn tool results into an Evidence Ledger, record failure downgrade paths, stop conditions, and final-output audit gates. `planner.json` and `todo.md` now record main/supporting dimensions, initial hypotheses, expected tool results, downstream use, failure fallbacks, stop conditions, and the replan policy. `todo.md` is written with the initial plan and rewritten at the end with tool success/failure status, arguments, result previews, and execute-review state.
