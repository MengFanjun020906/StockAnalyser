...
| `AGENT_CAPITAL_FLOW_TIMEOUT_SECONDS` | Timeout budget for explicit Agent `get_capital_flow` calls; the tool now combines Tushare `moneyflow_dc`, `moneyflow_ths`, and legacy `moneyflow` per-stock historical flow sources, with StockAPI `codeFlow` fallback | `15.0` | Optional |
