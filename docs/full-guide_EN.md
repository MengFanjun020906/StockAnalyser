...
| `AGENT_CAPITAL_FLOW_TIMEOUT_SECONDS` | Timeout budget for explicit Agent `get_capital_flow` calls; Tushare `moneyflow` is the current default per-stock historical capital-flow source, with StockAPI `codeFlow` fallback | `15.0` | Optional |
...
> - The Seed Pool quality page keeps one latest pool per `seed_date`. If the candidate pool is generated multiple times on the same day, the newer pool replaces the older one, and T+1 evaluation plus dashboard summaries use only the latest pool. Pools generated before 09:00 Beijing time are attributed to the previous natural day; for example, a pool generated before 09:00 on June 10 belongs to June 9.
...
> - `get_capital_flow` uses Tushare `moneyflow` by default. Tushare raw amount fields are in 10k CNY and the tool exposes CNY: `net_mf_amount` maps to all-order `net_inflow/net_inflow_5d/net_inflow_10d`, while `buy_lg_amount + buy_elg_amount - sell_lg_amount - sell_elg_amount` maps to main-force `main_net_inflow/main_inflow_5d/main_inflow_10d`; compatibility fields `inflow_5d/inflow_10d` now point to the main-force definition. When Tushare is unavailable it falls back to StockAPI historical `codeFlow`. Without `STOCKAPI_TOKEN`, the fallback can only query delayed history windows under the free quota and exposes the data date via `latest_date`.
