...
下表不展示最新单笔结果，而是汇总从首次决策日开始的累计表现，并和同期指数做对比。当前本地回测样本覆盖 48 个 Trace、94 条最终计划，决策日期为 2026-05-15 至 2026-07-14，最新样本退出时间为 2026-07-14 15:00:00；指数基准来自本地 `Sequoia-X/data/sequoia_v2.db`，截至最新可用交易日 2026-07-16。

![Entry execution backtest cumulative PnL vs benchmarks](docs/assets/backtest-pnl-benchmark.svg)

| 对象 | 类型 | 统计窗口 | 样本/成交 | 触发率 | 胜率 | 累计 PnL | Alpha vs 上证 | 平均/中位单笔 |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `strict_ai_entry` | AI 条件入场 | 2026-05-15 -> 2026-07-14 | 94 / 48 | 51.06% | 18.75% | -46.35% | -40.23pp | -1.09% / -2.45% |
| `next_open_baseline` | 次日开盘基准 | 2026-05-15 -> 2026-07-14 | 94 / 91 | 96.81% | 29.67% | -90.44% | -84.32pp | -1.81% / -3.40% |
| `atr_elastic_entry` | ATR 弹性入场 | 2026-05-15 -> 2026-07-14 | 94 / 55 | 58.51% | 18.18% | -77.43% | -71.31pp | -2.47% / -3.71% |
| 上证指数 `000001.SH` | 指数基准 | 2026-05-15 -> 2026-07-16 | - | - | - | -6.12% | 0.00pp | - |
| 沪深300 `000300.SH` | 指数基准 | 2026-05-15 -> 2026-07-16 | - | - | - | -3.32% | +2.80pp | - |

策略累计 PnL 使用已成交样本按顺序复利，未触发信号计入触发率但不进入收益复利；它是入场规则诊断口径，不是满仓组合净值。当前 AI 条件入场相对次日开盘基准减少了成交和亏损，但相对同期指数仍明显落后，后续重点应继续改进候选质量、入场有效期、资金确认和 risk-off 降级规则。回测只用于诊断，不构成收益承诺。

图表由 `scripts/render_backtest_readme_chart.py` 使用 `matplotlib` 生成；回测数据或指数库更新后重新运行该脚本即可刷新 README 曲线。
