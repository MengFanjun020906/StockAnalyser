...
_CATEGORY_DEFINITIONS: List[Dict[str, Any]] = [
    ...
    },
    {
        "category": "graphiti",
        "title": "Graphiti",
        "description": "Temporal knowledge graph integration settings.",
        "display_order": 58,
    },
    {
        "category": "backtest",
...
_FIELD_DEFINITIONS: Dict[str, Dict[str, Any]] = {
    ...
    "TUSHARE_HTTP_URL": {
        "title": "Tushare HTTP URL",
        "description": "Tushare-compatible API endpoint. Defaults to the project private gateway.",
        "category": "data_source",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
        "is_required": False,
        "is_editable": True,
        "default_value": "http://118.89.66.41:8010/",
        "options": [],
        "validation": {},
        "display_order": 11,
    },
    ...
    "ANYSEARCH_API_KEY": {
        ...
        "description": "Unified search API key used for stock news, market review, Agent search, and general web search.",
        "category": "data_source",
        "data_type": "string",
        "ui_control": "password",
        "is_sensitive": True,
    ...
    "ENABLE_CHIP_DISTRIBUTION": {
        ...
        "description": "Enable chip distribution analysis. The tool prefers Tushare cyq_chips and falls back to manager diagnostics when unavailable.",
        "category": "data_source",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "AGENT_CAPITAL_FLOW_TIMEOUT_SECONDS": {
        ...
        "description": "Timeout budget for the explicit Agent get_capital_flow tool. Tushare moneyflow is the default per-stock capital-flow source, with StockAPI codeFlow fallback.",
        "category": "data_source",
        "data_type": "float",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_TOOL_CALL_TIMEOUT_SECONDS": {
        "title": "Agent Tool Call Timeout Seconds",
        "description": "Max wall-clock seconds for one Agent tool call batch before degrading that batch to tool timeout.",
        "category": "agent",
        "data_type": "float",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_REGIME_COMPONENT_TIMEOUT_SECONDS": {
        "title": "Agent Regime Component Timeout Seconds",
        "description": "Max seconds for one detect_market_regime component. The default favors fresh auxiliary market-regime inputs such as sector rankings, northbound flow, margin and market capital flow.",
        "category": "agent",
        "data_type": "float",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_SECTOR_RANKINGS_TIMEOUT_SECONDS": {
        "title": "Agent Sector Rankings Timeout Seconds",
        "description": "Max seconds for sector-ranking data-source probing. detect_market_regime uses this as an inner timeout for sector breadth/context.",
        "category": "agent",
        "data_type": "float",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_TUSHARE_TOOL_TIMEOUT_SECONDS": {
        "title": "Agent Tushare Tool Timeout Seconds",
        "description": "Max seconds for one Agent-side Tushare request. Shared by market-regime auxiliary components, margin, capital-flow and other Tushare-backed tools.",
        "category": "agent",
        "data_type": "float",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_SEED_FACT_MAX_WORKERS": {
        "title": "Agent Seed Fact Max Workers",
        "description": "Max concurrent (seed, tool) workers for pre-desk SeedFactPacket collection in thesis-desk candidate discovery.",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_SEED_FACT_TOOL_TIMEOUT_SECONDS": {
        "title": "Agent Seed Fact Tool Timeout Seconds",
        "description": "Timeout budget for one pre-desk SeedFactPacket tool call. Failures remain trace-visible and do not become silent successful facts.",
        "category": "agent",
        "data_type": "float",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_CANDIDATE_BLACKLIST_CODES": {
        "title": "Agent Candidate Blacklist Codes",
        "description": "Comma-separated stock codes hard-excluded from L1 candidate pools before expert merge and Judge evaluation.",
        "category": "agent",
        "data_type": "string",
        "ui_control": "textarea",
        "is_sensitive": False,
    ...
    "AGENT_SELECTION_DEEP_DIVE_LIMIT": {
        "title": "Agent Selection Deep Dive Limit",
        "description": "Maximum L1 watchlist candidates that enter stock-level deep-dive evidence collection. Valid range is 1-5; higher values improve coverage but call more tools.",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_DEEP_DIVE_SETUP_ROUTER_ENABLED": {
        "title": "Agent Deep Dive Setup Router Enabled",
        "description": "Route single-stock deep-dive prompts by setup_type playbook (early_turn / trend_continuation / capital_momentum / quality_repair / theme_follow; unknown falls back to the conservative generic template). Disable to use the legacy single deep-dive prompt.",
        "category": "agent",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "AGENT_VETO_GATE_ENABLED": {
        "title": "Agent Veto Gate Enabled",
        "description": "Enable the shared bearish red-line veto gate (thesis_desk_committee selection mode). By default only non-empty hard_risk_flags (ST/delist/suspended/major un-clarified bad news) trigger a veto. Disable to pass all candidates through.",
        "category": "agent",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "AGENT_VETO_VIOLENT_OUTFLOW_THRESHOLD": {
        "title": "Agent Veto Violent Outflow Threshold",
        "description": "Net-outflow strength (%) at/above which an outflow stock is flagged capital_violent_outflow and vetoed. Leave empty to disable (never triggers) — avoids killing clean low-base picks where capital has not yet entered.",
        "category": "agent",
        "data_type": "float",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_VETO_BREAKDOWN_ACCEL_THRESHOLD": {
        "title": "Agent Veto Breakdown Accel Threshold",
        "description": "5-day drop magnitude (%) at/above which a bearish stock trading below MA20 is flagged breakdown_accelerating and vetoed. Leave empty to disable (never triggers).",
        "category": "agent",
        "data_type": "float",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    # P3 recall layer -----------------------------------------------------------
    "AGENT_RECALL_COARSE_CAP": {
        "title": "Agent Recall Coarse Cap",
        "description": "Maximum number of FeatureRows kept after the coarse-filter step in the P3 recall layer (thesis_desk_committee mode). Rows are ranked by hit-count (how many detectors fired); ties at the boundary are always fully preserved. This cap is a recall-completeness safety valve, not a ranking mechanism.",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_DESK_FALLBACK_SUPPLEMENT_N": {
        "title": "Agent Desk Fallback Supplement N",
        "description": "When a thesis desk's input subset (filtered by eligibility rules) is empty, this many additional rows from the full recall pool are added as a fallback supplement, preventing empty desk runs.",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "DESK_BACKFILL_RULES_JSON": {
        "title": "Desk Backfill Rules JSON",
        "description": "JSON mapping regime → {donor_desk: [recipient_desks]}. Controls how empty slots are redistributed across desks. Defensive regimes (risk_off/panic/trending_down) never backfill to momentum_desk regardless of this config.",
        "category": "agent",
        "data_type": "string",
        "ui_control": "textarea",
        "is_sensitive": False,
    ...
    "DESK_PICK_TOP_N": {
        "title": "Desk Pick Top N",
        "description": "Maximum candidates each desk can contribute before regime slot allocation. Caps per-desk output to prevent one dominant desk from flooding the pool.",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "SELECTION_TOTAL_SLOTS": {
        "title": "Selection Total Slots",
        "description": "Total number of final candidates output by allocate_slots() across all desks. This is the L1 pool size fed to the deep-dive layer.",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "DESK_BACKFILL_MAX": {
        "title": "Desk Backfill Max",
        "description": "Maximum number of extra candidates that can be added per backfill step when a desk under-delivers its quota.",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "DESK_MAX_LLM_ROUNDS": {
        "title": "Desk Max LLM Rounds",
        "description": "Maximum LLM conversation rounds per thesis desk. Controls cost vs. thoroughness tradeoff.",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "DESK_MAX_TOOL_CALLS": {
        "title": "Desk Max Tool Calls",
        "description": "Maximum tool invocations per thesis desk run.",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_CANDIDATE_MIN_AVG_AMOUNT": {
        "title": "Agent Candidate Min Avg Amount",
        "description": "Minimum liquidity threshold for candidate pools when a provider supplies avg amount/turnover fields. Set 0 to disable.",
        "category": "agent",
        "data_type": "float",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_CANDIDATE_MIN_LISTING_DAYS": {
        "title": "Agent Candidate Min Listing Days",
        "description": "Minimum listing age for candidate pools when a provider supplies listing days. Set 0 to disable.",
        "category": "agent",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "AGENT_CANDIDATE_ENFORCE_NAME_CODE_MATCH": {
        "title": "Agent Candidate Enforce Name-Code Match",
        "description": "Exclude candidates whose provided stock name conflicts with local stock master data for the same code.",
        "category": "agent",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "AGENT_FUNDAMENTAL_CANDIDATE_DB_PATH": {
        "title": "Agent Fundamental Candidate DB Path",
        "description": "SQLite DB path for precomputed fundamental_candidate_snapshot rows. Empty reuses DATABASE_PATH.",
        "category": "agent",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "AGENT_CANDIDATE_POOL_DB_PATH": {
        "title": "Agent Candidate Pool DB Path",
        "description": "SQLite DB path for persisted Agent candidate-pool runs. Empty reuses DATABASE_PATH.",
        "category": "agent",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "STOCKAPI_TOKEN": {
        ...
        "description": "Optional token for stockapi.com.cn codeFlow fallback used by get_capital_flow when Tushare moneyflow is unavailable. Blank uses the limited free quota, which only supports delayed history windows.",
        "category": "data_source",
        "data_type": "string",
        "ui_control": "password",
        "is_sensitive": True,
    ...
    "STOCKAPI_URL": {
        "title": "StockAPI codeFlow URL",
        "description": "StockAPI historical stock capital-flow endpoint used as fallback for per-stock money-flow queries.",
        "category": "data_source",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
        "is_required": False,
        "is_editable": True,
        "default_value": "https://www.stockapi.com.cn/v1/base/codeFlow",
        "options": [],
        "validation": {},
        "display_order": 26,
    },
    ...
    "NEWS_EVENT_EXTRACTOR_MODE": {
        "title": "News Event Extractor Mode",
        "description": "Optional structured news event extractor mode. fallback uses deterministic rules only; llm uses LiteLLM JSON extraction with rule fallback; auto uses LLM only when runtime config is available.",
        "category": "data_source",
        "data_type": "string",
        "ui_control": "select",
        "is_sensitive": False,
        ...
        "options": ["fallback", "llm", "auto"],
        "validation": {"enum": ["fallback", "llm", "auto"]},
        "display_order": 62,
    },
    "NEWS_EVENT_EXTRACTOR_MODEL": {
        "title": "News Event Extractor Model",
        "description": "LiteLLM model used for structured news event extraction. Recommended: deepseek/deepseek-v4-flash.",
        "category": "data_source",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "NEWS_EVENT_EXTRACTOR_TIMEOUT_SECONDS": {
        "title": "News Event Extractor Timeout",
        "description": "Timeout in seconds for one lightweight LLM news event extraction request.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "NEWS_EVENT_EXTRACTOR_MAX_TOKENS": {
        "title": "News Event Extractor Max Tokens",
        "description": "Maximum output tokens for one structured news event extraction request.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
        ...
        "default_value": "900",
        "options": [],
        "validation": {"min": 128, "max": 4096},
        "display_order": 65,
    },
    "NEWS_EVENT_EXTRACTOR_TEMPERATURE": {
        "title": "News Event Extractor Temperature",
        "description": "Sampling temperature for structured news event extraction. Keep at 0 for deterministic JSON extraction.",
        "category": "data_source",
        "data_type": "float",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "GRAPHITI_ENABLED": {
        "title": "Enable Graphiti",
        "description": "Enable temporal knowledge graph ingestion and retrieval. Disabled by default and incurs no runtime overhead.",
        "category": "graphiti",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "NEO4J_URI": {
        "title": "Neo4j URI",
        "description": "Neo4j Bolt URI for Graphiti, e.g. bolt://localhost:7687.",
        "category": "graphiti",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "NEO4J_USER": {
        "title": "Neo4j User",
        "description": "Neo4j username for Graphiti.",
        "category": "graphiti",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "NEO4J_PASSWORD": {
        ...
        "description": "Neo4j password for Graphiti.",
        "category": "graphiti",
        "data_type": "string",
        "ui_control": "password",
        "is_sensitive": True,
    ...
    "GRAPHITI_LLM_MODEL": {
        "title": "Graphiti LLM Model",
        "description": "Optional Graphiti-specific LLM model. Empty means reuse the main LiteLLM model.",
        "category": "graphiti",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "GRAPHITI_EMBEDDING_MODEL": {
        "title": "Graphiti Embedding Model",
        "description": "Optional Graphiti embedding model. Empty uses OpenAI-compatible embeddings when a key exists, otherwise falls back to local hash embeddings for best-effort graph sync.",
        "category": "graphiti",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "GRAPHITI_EMBEDDING_BASE_URL": {
        "title": "Graphiti Embedding Base URL",
        "description": "Optional OpenAI-compatible embedding API base URL for Graphiti.",
        "category": "graphiti",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
    ...
    "GRAPHITI_EMBEDDING_API_KEY": {
        "title": "Graphiti Embedding API Key",
        "description": "Optional embedding API key for Graphiti. Use this when embeddings should use a different provider or key from the main LLM.",
        "category": "graphiti",
        "data_type": "string",
        "ui_control": "password",
        "is_sensitive": True,
    ...
    "GRAPHITI_GROUP_STRATEGY": {
        "title": "Graphiti Group Strategy",
        "description": "Graph partition strategy: market, user, or single.",
        "category": "graphiti",
        "data_type": "string",
        "ui_control": "select",
        "is_sensitive": False,
        ...
        "options": ["market", "user", "single"],
        "validation": {"enum": ["market", "user", "single"]},
        "display_order": 70,
    },
    "GRAPHITI_OUTBOX_WORKER_ENABLED": {
        "title": "Graphiti Outbox Worker",
        "description": "Process durable Graphiti projection jobs in schedule mode without blocking relational writes.",
        "category": "graphiti",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "GRAPHITI_OUTBOX_INTERVAL_SECONDS": {
        "title": "Outbox Poll Interval",
        "description": "Seconds between Graphiti outbox worker polls in schedule mode.",
        "category": "graphiti",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "GRAPHITI_OUTBOX_BATCH_SIZE": {
        "title": "Outbox Batch Size",
        "description": "Maximum durable Graphiti jobs processed per worker poll.",
        "category": "graphiti",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "GRAPHITI_OUTBOX_MAX_ATTEMPTS": {
        "title": "Outbox Max Attempts",
        "description": "Maximum attempts before a Graphiti outbox job is marked dead.",
        "category": "graphiti",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "GRAPHITI_OUTBOX_RETRY_BASE_SECONDS": {
        "title": "Outbox Retry Base",
        "description": "Base seconds for exponential Graphiti outbox retry backoff.",
        "category": "graphiti",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "GRAPHITI_OUTBOX_JOB_TIMEOUT_SECONDS": {
        "title": "Outbox Job Timeout",
        "description": "Hard timeout for one Graphiti episode projection before the job enters retry backoff.",
        "category": "graphiti",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "GRAPHITI_SELECTION_SEARCH_TIMEOUT_SECONDS": {
        "title": "Selection Graph Search Timeout",
        "description": "Hard timeout for required Prompt 1 and Prompt 5 graph evidence retrieval before relational fallback.",
        "category": "graphiti",
        "data_type": "float",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "NEWS_SIGNAL_EMBEDDING_THRESHOLDS_JSON": {
        "title": "News Semantic Threshold Profiles",
        "description": "JSON object mapping embedding model names to semantic edge thresholds.",
        "category": "graphiti",
        "data_type": "json",
        "ui_control": "textarea",
        "is_sensitive": False,
        "is_required": False,
        "is_editable": True,
        "default_value": "{\"default\":0.78,\"mxbai-embed-large\":0.76}",
        "options": [],
        "validation": {},
        "display_order": 92,
    },
    "NEWS_SIGNAL_CLS_INCREMENTAL_ENABLED": {
        "title": "CLS Incremental Polling",
        "description": "Enable idempotent CLS news polling in schedule mode.",
        "category": "data_source",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "NEWS_SIGNAL_CLS_INCREMENTAL_INTERVAL_MINUTES": {
        "title": "CLS Poll Interval",
        "description": "CLS incremental polling interval in minutes; constrained to 5-10 minutes.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "NEWS_SIGNAL_CLS_INCREMENTAL_LIMIT": {
        "title": "CLS Poll Limit",
        "description": "Maximum ranked CLS feed items inspected per incremental poll.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "NEWS_SIGNAL_PORTFOLIO_ANYSEARCH_ENABLED": {
        "title": "Portfolio AnySearch News",
        "description": "Enable holdings-only AnySearch message ingestion. This writes news signal cards and does not run analysis reports.",
        "category": "data_source",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "NEWS_SIGNAL_PORTFOLIO_ANYSEARCH_INTERVAL_MINUTES": {
        "title": "Portfolio AnySearch Interval",
        "description": "Holdings news search interval in minutes; default is 5 hours.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "NEWS_SIGNAL_PORTFOLIO_ANYSEARCH_MAX_RESULTS_PER_STOCK": {
        "title": "Portfolio AnySearch Results",
        "description": "Maximum AnySearch results requested per real holding.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "NEWS_SIGNAL_PORTFOLIO_ANYSEARCH_MAX_AGE_DAYS": {
        "title": "Portfolio AnySearch Max Age",
        "description": "Drop dated holdings search results older than this many days; undated actionable announcements are retained.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "NEWS_SIGNAL_PORTFOLIO_ANYSEARCH_RUN_IMMEDIATELY": {
        "title": "Run Portfolio Search Immediately",
        "description": "Run the holdings AnySearch message task once when the news worker starts.",
        "category": "data_source",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "NEWS_EVENT_SENTINEL_ENABLED": {
        "title": "News Event Sentinel",
        "description": "Enable opt-in watched-universe news event sentinel polling in schedule mode.",
        "category": "data_source",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "NEWS_EVENT_SENTINEL_INTERVAL_MINUTES": {
        "title": "Sentinel Poll Interval",
        "description": "News event sentinel polling interval in minutes; constrained to 5-120 minutes.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "NEWS_EVENT_SENTINEL_ACTIVE_WINDOWS": {
        "title": "Sentinel Active Windows",
        "description": "Local Asia/Shanghai active window string for the news event sentinel, e.g. 08:00-23:59.",
        "category": "data_source",
        "data_type": "string",
        "ui_control": "text",
        "is_sensitive": False,
        "is_required": False,
        "is_editable": True,
        "default_value": "08:00-23:59",
        "options": [],
        "validation": {},
        "display_order": 72,
    },
    "NEWS_EVENT_SENTINEL_MAX_ITEMS_PER_SOURCE": {
        "title": "Sentinel Source Item Limit",
        "description": "Maximum items inspected per source by one sentinel run.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "NEWS_EVENT_SENTINEL_CARD_MAX_AGE_MINUTES": {
        "title": "Sentinel Card Max Age",
        "description": "Maximum age in minutes for a news signal card to be eligible for market-trigger notifications.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "NEWS_EVENT_SENTINEL_MIN_SEVERITY": {
        "title": "Sentinel Minimum Severity",
        "description": "Minimum event severity that may emit a notification envelope.",
        "category": "data_source",
        "data_type": "string",
        "ui_control": "select",
        "is_sensitive": False,
        ...
        "validation": {"enum": ["low", "mid", "high", "critical"]},
        "display_order": 75,
    },
    "NEWS_EVENT_SENTINEL_COOLDOWN_MINUTES": {
        "title": "Sentinel Cooldown",
        "description": "Minutes to suppress duplicate notifications for the same symbol/event/direction.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "NEWS_EVENT_SENTINEL_TRIGGER_MODE": {
        "title": "Sentinel Trigger Mode",
        "description": "First rollout uses notify_only; Agent Trace trigger modes are reserved for later guarded rollout.",
        "category": "data_source",
        "data_type": "string",
        "ui_control": "select",
        "is_sensitive": False,
        "is_required": False,
        "is_editable": True,
        "default_value": "notify_only",
        "options": ["notify_only", "trace_for_holdings", "manual_confirm", "disabled"],
        "validation": {"enum": ["notify_only", "trace_for_holdings", "manual_confirm", "disabled"]},
        "display_order": 77,
    },
    "NEWS_EVENT_SENTINEL_TRACE_MAX_PER_RUN": {
        "title": "Sentinel Trace Max Per Run",
        "description": "Reserved Agent Trace budget per sentinel run; not used by notify_only mode.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "NEWS_EVENT_SENTINEL_TRACE_MAX_PER_DAY": {
        "title": "Sentinel Trace Max Per Day",
        "description": "Reserved daily Agent Trace budget for later sentinel Trace trigger rollout.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
    ...
    "NEWS_EVENT_SENTINEL_RUN_IMMEDIATELY": {
        "title": "Run Sentinel Immediately",
        "description": "Run the sentinel once when schedule mode starts. Keep false in production to avoid surprise bursts.",
        "category": "data_source",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "NEWS_EVENT_SENTINEL_FEISHU_ENABLED": {
        "title": "Sentinel Feishu Webhook",
        "description": "Send triggered news event sentinel envelopes to the configured Feishu custom bot webhook. Uses FEISHU_WEBHOOK_URL / FEISHU_WEBHOOK_SECRET / FEISHU_WEBHOOK_KEYWORD.",
        "category": "data_source",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "NEWS_EVENT_SENTINEL_HEARTBEAT_ENABLED": {
        ...
        "description": "Send a low-severity liveness card when a sentinel run scans successfully but emits no market trigger.",
        "category": "data_source",
        "data_type": "boolean",
        "ui_control": "switch",
        "is_sensitive": False,
    ...
    "NEWS_EVENT_SENTINEL_HEARTBEAT_INTERVAL_MINUTES": {
        "title": "Sentinel Heartbeat Interval",
        "description": "Minimum minutes between no-trigger sentinel heartbeat notifications; constrained to 5-1440 minutes.",
        "category": "data_source",
        "data_type": "integer",
        "ui_control": "number",
        "is_sensitive": False,
